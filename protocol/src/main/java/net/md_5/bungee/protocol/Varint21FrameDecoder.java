...
import online.nasgar.antibot.utils.FastCorruptedFrameException;
...
public class Varint21FrameDecoder extends ByteToMessageDecoder
{
    ...
    {
        // AntiBot start - rewrite Varint21Decoder
        if ( !ctx.channel().isActive() ) // AntiBot - added this if statament
        {
        ...
        while ( i-- > 0 )
        {
            ...
            if ( read >= 0 )
            {
                ...
                if ( packetLength <= 0 && !fromBackend ) // AntiBot dont throw exception for empties packets from backend
                {
                    super.setSingleDecode( true );  // AntiBot
        ...
        super.setSingleDecode( true ); // AntiBot
        throw new FastCorruptedFrameException( "length wider than 21-bit" ); // AntiBot
    }
