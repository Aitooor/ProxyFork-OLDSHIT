...
import online.nasgar.antibot.utils.FastException;
import online.nasgar.antibot.utils.IPUtils;
import online.nasgar.antibot.utils.PingLimiter;
...
public class InitialHandler extends PacketHandler implements PendingConnection
{
    ...
    {
        if ( packet.packet == null && !ch.isClosed() ) //AntiBot ch.isClosed
        {
            throw new QuietException( "Unexpected packet received during login process! " ); //AntiBot removed bump
        }
    }
    ...
    {
        checkState( !this.legacy, "Multiple legacy pings" ); //AntiBot
        this.legacy = true;
        final boolean v1_5 = ping.isV1_5();

        ServerPing legacy = new ServerPing( new ServerPing.Protocol( bungee.getCustomBungeeName(), bungee.getProtocolVersion() ), //AntiBot
                new ServerPing.Players( listener.getMaxPlayers(), bungee.getOnlineCountBF( true ), null ), //AntiBot
    ...
    private ServerPing getPingInfo(String motd, int protocol)
    {
        return new ServerPing(
                new ServerPing.Protocol( bungee.getCustomBungeeName(), protocol ), //AntiBot
                new ServerPing.Players( listener.getMaxPlayers(), bungee.getOnlineCountBF( true ), null ), //AntiBot
                motd, PingLimiter.handle() ? null : BungeeCord.getInstance().config.getFaviconObject() //AntiBot PingLimiter.handle() ? null :
        );
    }
    ...
    {
        checkState( thisState == State.STATUS, "Not expecting STATUS" ); //AntiBot
    ...
    {
        checkState( thisState == State.PING, "Not expecting PING" ); //AntiBot
    ...
    {
        checkState( thisState == State.HANDSHAKE, "Not expecting HANDSHAKE" ); //AntiBot
        ...
        switch ( handshake.getRequestedProtocol() )
        {
            case 1:
                ...
                {
                    bungee.getLogger().log( Level.INFO, "{0} has pinged", this.toString() ); // AntiBot, use toString()
                }
                thisState = State.STATUS;
                ch.setProtocol( Protocol.STATUS );
                bungee.getAntiBot().getServerPingUtils().add( getAddress().getAddress() ); //AntiBot
                break;
            case 2:
                // Login
                bungee.getLogger().log( Level.INFO, "{0} has connected", this.toString() ); // AntiBot, use toString()
                ...
                break;
            default:
                throw new FastException( "[" + ch.getHandle().remoteAddress() + "] Cannot request protocol " + handshake.getRequestedProtocol() ); //AntiBot
        }
    }
    ...
    {
        checkState( thisState == State.USERNAME, "Not expecting USERNAME" ); //AntiBot
        ...
        bungee.getAntiBot().checkAsyncIfNeeded( this );
        //AntiBot moved code to delayedHandleOfLoginRequset();
    }
    ...
    public void delayedHandleOfLoginRequset()
    {
        ...
        if ( limit > 0 && bungee.getOnlineCountBF( false ) >= limit )//AntiBot
        {
    ...
    {
        checkState( thisState == State.ENCRYPT, "Not expecting ENCRYPT" ); //AntiBot
    ...
    private void finish()
    {
        ...
        //AntiBot start
        if ( bungee.getAntiBot().isOnChecking( getName() ) )
        {
            disconnect( bungee.getTranslation( "already_connected_proxy" ) ); // TODO: Cache this disconnect packet
            return;
        }
        ...
        //Because AntiBot delayed a LoginEvent when player needs a check for a bot,
        //plugins can not change a uniqueId field via reflection in event, because AntiBot needs send
        ...
        if ( bungee.getAntiBot().needCheck( this ) )
        {
            sendLoginSuccess( !sendLoginSuccess ); //Send a loginSuccess if sendLoginSuccess is false
            bungee.getAntiBot().connectToAntiBot( userCon );
        } else
        {
            bungee.getAntiBot().saveUser( userCon.getName().toLowerCase(), IPUtils.getAddress( userCon ), false ); //update timestamp
    ...
    private void finnalyFinishLogin(UserConnection userCon, boolean ignoreLoginSuccess)
    {
        ...
        ch.getHandle().pipeline().get( HandlerBoss.class ).setHandler( new UpstreamBridge( bungee, userCon ) ); //AntiBot

        bungee.getPluginManager().callEvent( new PostLoginEvent( userCon ) ); //AntiBot
