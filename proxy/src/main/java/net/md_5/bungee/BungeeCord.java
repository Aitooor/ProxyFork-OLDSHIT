...
import online.nasgar.antibot.AntiBotThread;
import online.nasgar.antibot.config.Settings;
import online.nasgar.antibot.utils.FakeOnlineUtils;
...
public class BungeeCord extends ProxyServer
{
    ...
    public EventLoopGroup bossEventLoopGroup, workerEventLoopGroup, queryEventLoopGroup; //AntiBot
    /**
    ...
    @Getter
    private String customBungeeName; //AntiBot

    @Getter
    @Setter
    private AntiBot antiBot; //AntiBot
    ...
    public BungeeCord() throws IOException
    {
        ...
        getPluginManager().registerCommand( null, new CommandReloadServers() ); //AntiBot
    ...
    public void start() throws Exception
    {
        ...
        String nameProperty = System.getProperty( "bungeeName" ); // AntiBot
        customBungeeName = ( nameProperty == null ? getName() : nameProperty ) + " " + getGameVersion(); // AntiBot

        this.antiBot = new AntiBot( true ); //Hook AntiBot into Bungee
        new FakeOnlineUtils(); //Init fake online
        AntiBotThread.startCleanUpThread(); //AntiBot

        bossEventLoopGroup = PipelineUtils.newEventLoopGroup( 0, new ThreadFactoryBuilder().setNameFormat( "Netty Boss IO Thread #%1$d" ).build() ); //AntiBot //WaterFall backport
        eventLoops = workerEventLoopGroup = PipelineUtils.newEventLoopGroup( 0, new ThreadFactoryBuilder().setNameFormat( "Netty Worker IO Thread #%1$d" ).build() ); //AntiBot //WaterFall backport
        queryEventLoopGroup = PipelineUtils.newEventLoopGroup( 1, new ThreadFactoryBuilder().setNameFormat( "Query Netty IO Thread #%1$d" ).build() ); //AntiBot
    ...
    public void startListeners()
    {
        for ( final ListenerInfo info : config.getListeners() )
        {
            ...
            new ServerBootstrap()
                    ...
                    .group( bossEventLoopGroup, workerEventLoopGroup ) //AntiBot //WaterFall backport
            ...
            if ( info.isQueryEnabled() )
            {
                ...
                new RemoteQuery( this, info ).start( PipelineUtils.getDatagramChannel(), new InetSocketAddress( info.getHost().getAddress(), info.getQueryPort() ), queryEventLoopGroup, bindListener ); //AntiBot
            }
        }
    }
    ...
    private void independentThreadStop(final String reason, boolean callSystemExit)
    {
        ...
        bossEventLoopGroup.shutdownGracefully(); //AntiBot //WaterFall backport
        workerEventLoopGroup.shutdownGracefully(); //AntiBot //WaterFall backport
        queryEventLoopGroup.shutdownGracefully(); //AntiBot
        while ( true ) //AntiBot //WaterFall backport {
            try
            {
                bossEventLoopGroup.awaitTermination( Long.MAX_VALUE, TimeUnit.NANOSECONDS ); //AntiBot //WaterFall backport
                workerEventLoopGroup.awaitTermination( Long.MAX_VALUE, TimeUnit.NANOSECONDS ); //AntiBot //WaterFall backport
                queryEventLoopGroup.awaitTermination( Long.MAX_VALUE, TimeUnit.NANOSECONDS ); //AntiBot
                break;
    ...
    public int getOnlineCountBF(boolean fake)
    {
        ...
        {
            online += antiBot.getOnlineOnFilter();
        }
