...
{
    //AntiBot start - limit compressed data to 2 MiB
    ...
    static
    {
        BungeeCord.getInstance().getLogger().info( "[AntiBot] Maximum packet size: " + MAXIMUM_UNCOMPRESSED_SIZE );
        BungeeCord.getInstance().getLogger().info( "[AntiBot] Maximum packet size while checking: " + MAXIMUM_UNCOMPRESSED_SIZE_WHILE_CHECKING );
    }
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception
    {
        ...
        } else
        {
            ...
            try
            {
                zlib.process( in, decompressed, checking ); //AntiBot
