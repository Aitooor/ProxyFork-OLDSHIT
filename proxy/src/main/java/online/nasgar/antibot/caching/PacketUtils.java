package online.nasgar.antibot.caching;
...
import online.nasgar.antibot.packets.EmptyChunkPacket;
import online.nasgar.antibot.packets.JoinGame;
import online.nasgar.antibot.packets.PlayerAbilities;
import online.nasgar.antibot.packets.PlayerPositionAndLook;
import online.nasgar.antibot.packets.SetExp;
import online.nasgar.antibot.packets.SetSlot;
import online.nasgar.antibot.packets.TimeUpdate;
...
public class PacketUtils
{
    ...
    public static void init()
    {
        ...
        DefinedPacket[] packets =
        {
            ...
            createMessagePacket( Settings.IMP.MESSAGES.CHECKING_CAPTCHA_WRONG.replaceFirst( "%s", "2" ).replaceFirst( "%s", "attempts" ) ), //8
            createMessagePacket( Settings.IMP.MESSAGES.CHECKING_CAPTCHA_WRONG.replaceFirst( "%s", "1" ).replaceFirst( "%s", "attempts" ) ), //9
        ...
        {
            PacketUtils.cachedPackets[i] = new CachedPacket( packets[i], Protocol.AntiBot, Protocol.GAME );
        }
        ...
        kickMessagesGame.put( KickType.BIG_PACKET, new CachedPacket( createKickPacket( "§cFailed to check for bot. Please inform the administration about this. (a large package was sent)" ), kickGame ) );
    ...
    private static DefinedPacket createPluginMessage()
    {
        ...
        DefinedPacket.writeString( "AntiBot (https://vk.cc/8hr1pU)", brand );
