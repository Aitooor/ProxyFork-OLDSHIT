...
import online.nasgar.antibot.caching.CachedCaptcha;
import online.nasgar.antibot.caching.PacketUtils;
import online.nasgar.antibot.caching.PacketUtils.KickType;
import online.nasgar.antibot.captcha.CaptchaGeneration;
import online.nasgar.antibot.config.Settings;
import online.nasgar.antibot.utils.GeoIp;
import online.nasgar.antibot.utils.ManyChecksUtils;
import online.nasgar.antibot.utils.ServerPingUtils;
import online.nasgar.antibot.utils.Sql;
...
public class AntiBot
{
    ...
    //UserName, Ip
    @Getter
    private final Map<String, AntiBotUser> userCache = new ConcurrentHashMap<>();
    ...
    public AntiBot(boolean startup)
    {
        Settings.IMP.reload( new File( "AntiBot", "config.yml" ) );
    ...
    public void disable()
    {
        ...
        for ( Connector connector : connectedUsersSet.values() )
        {
            ...
            {
                connector.getUserConnection().disconnect( "§c[AntiBot] §aПерезагрузка фильтра" );
            }
    ...
    public void saveUser(String userName, InetAddress address, boolean afterCheck)
    {
        ...
        AntiBotUser antiBotUser = userCache.get( userName );
        if ( antiBotUser == null )
        {
            antiBotUser = new AntiBotUser( userName, address.getHostAddress(), timestamp, timestamp );
        } else
        {
            antiBotUser.setIp( address.getHostAddress() );
            antiBotUser.setLastJoin( timestamp );
            if ( afterCheck )
            {
                antiBotUser.setLastCheck( timestamp );
            }
        }

        userCache.put( userName, antiBotUser);
        if ( sql != null )
        {
            sql.saveUser(antiBotUser);
        }
    }

    public void addUserToCache(AntiBotUser antiBotUser)
    {
        userCache.put( antiBotUser.getName(), antiBotUser);
    }
    ...
    public void connectToAntiBot(UserConnection userConnection)
    {
        userConnection.getCh().setEncoderProtocol( Protocol.GAME );
        userConnection.getCh().setDecoderProtocol( Protocol.AntiBot );
    ...
    {
        AntiBotUser antiBotUser = userCache.get( userName.toLowerCase() );
        if ( Settings.IMP.PROTECTION.CHECK_LOCALHOST >= 1 && address.isLoopbackAddress() )
        {
            return Settings.IMP.PROTECTION.CHECK_LOCALHOST != 1;
        }
        return antiBotUser == null || ( Settings.IMP.FORCE_CHECK_ON_ATTACK && isUnderAttack() )
            || !antiBotUser.getIp().equalsIgnoreCase( address.getHostAddress() );
    }
    ...
    public void checkAsyncIfNeeded(InitialHandler handler)
    {
        ...
        {
            PacketUtils.kickPlayer( KickType.THROTTLE, Protocol.LOGIN, ch, version ); //AntiBot
    ...
    private void checkForUpdates(boolean startup)
    {
        ...
        try
        {
            logger.log( Level.INFO, "[AntiBot] Проверяю наличее обновлений" );
                    ...
                    new InputStreamReader( conn.getInputStream() ) ) )
            {
                if ( !in.readLine().trim().equalsIgnoreCase( Settings.IMP.BOT_FILTER_VERSION ) )
                {
                    ...
                    logger.log( Level.INFO, "§c[AntiBot] §aНайдена новая версия!" );
                    logger.log( Level.INFO, "§c[AntiBot] §aПожалуйста обновитесь!" );
                    logger.log( Level.INFO, "§c[AntiBot] §ahttp://rubukkit.org/threads/137038" );
                    if ( startup )
                    {
                        Thread.sleep( 3500L );
                    }
                ...
                {
                    logger.log( Level.INFO, "[AntiBot] Обновлений не найдено!" );
                }
            }
        } catch ( IOException | InterruptedException ex )
        {
            logger.log( Level.WARNING, "[AntiBot] Не могу проверить обновление", ex );
        }
    }
