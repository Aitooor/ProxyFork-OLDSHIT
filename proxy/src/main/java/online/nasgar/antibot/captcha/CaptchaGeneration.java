package online.nasgar.antibot.captcha;
...
import online.nasgar.antibot.captcha.generator.CaptchaPainter;
import online.nasgar.antibot.captcha.generator.map.CraftMapCanvas;
import online.nasgar.antibot.captcha.generator.map.MapPalette;
import online.nasgar.antibot.packets.MapDataPacket;
...
public class CaptchaGeneration
{
    ...
    public void generateImages()
    {
        ...
        {
            BungeeCord.getInstance().getLogger().log( Level.INFO, "[AntiBot] Generating Captcha [{0}/900]", 900 - ex.getQueue().size() - ex.getActiveCount() );
            try
            {
                Thread.sleep( 1000L );
            } catch ( InterruptedException ex1 )
            {
                BungeeCord.getInstance().getLogger().log( Level.WARNING, "[AntiBot] Can't generate captcha. I turn off the bungee", ex1 );
                System.exit( 0 );
                return;
            }
        }
        ...
        System.gc();
        BungeeCord.getInstance().getLogger().log( Level.INFO, "[AntiBot] Captcha generated for {0} мс", System.currentTimeMillis() - start );
    }
